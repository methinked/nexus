...
**Date:** 2025-12-26  
**Controller:** 192.168.1.225 (Moria-Pi)  
**Total Nodes:** 3  
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **moria-pi** | Core + Agent | `192.168.1.225` | `10.243.14.179` | Pi 3B+ | External HDD (USB) | ✅ Online |
| **bywater-pi** | Agent | `192.168.1.175` | - | Pi 4 | **SD Card (Risk)** | ✅ Online |
| **orthanc-pi** | Agent | `192.168.1.222` | `10.243.151.228`| Pi 5 (NVMe) | NVMe SSD | ✅ Online |

## 2. Node Functionality & Status

### 2.1 Moria-Pi (The Core)
- **Function**: Runs the Nexus Core server and an Agent.
- **Functionality**:
    - Core API accessible at `http://192.168.1.225:8000`.
    - Agent metrics reporting correctly.
    - Hosting Docker containers: `scriba`, `bootpc`.
- **Storage**: Uses External HDD (`/dev/sda1`) for Docker root. **Good.**
- **Issues**: None currently detected.
...
### 2.3 Orthanc-Pi (The Powerhouse)
- **Function**: Standard Agent node (High Performance).
- **Functionality**:
    - Agent metrics reporting correctly.
    - Hosting `unifi-controller`.
